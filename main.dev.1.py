...
def main():
    ...
    # すべての束の情報を取得
    objPath_Json = Path(__file__).parent / f'test/supabase-test-data/bunches.json'
    with objPath_Json.open('rt') as objFile_Json:
        lst_Bunches: list = json.load(objFile_Json)
    for int_DayId in range(7):
        str_UpdatedAt_Raw: str = lst_Bunches[int_DayId]['updated_at']
    ...
    for int_DayId, dct_Day in enumerate(lst_Days):
        # 曜日に対応する束のカードを取得
        objPath_Json = Path(__file__).parent / f'test/supabase-test-data/bunch{int_DayId}.json'
        with objPath_Json.open('rt') as objFile_Output:
            lst_Cards: list = json.load(objFile_Output)
